...
class AduibRpcStarletteApp(JsonRpcApp):
    ...
    def add_routes(self, app: Starlette, rpc_path: str = DEFAULT_RPC_PATH) -> None:
        """Adds the RPC routes to the Starlette application.
        Args:
            app: The Starlette application instance.
            rpc_path: The path to mount the RPC endpoint.
        """
        app.routes.extend([Route(rpc_path, self._handle_requests, methods=["POST"], name="aduib_rpc_handler")])
        logger.debug(f"Added RPC route at path: {rpc_path}")

    def build(self, rpc_path: str = DEFAULT_RPC_PATH, **kwargs: Any) -> Starlette:
